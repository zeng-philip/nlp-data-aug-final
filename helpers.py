...
from textattack.transformations.word_innerswap_random import WordInnerSwapRandom
...
def prepare_train_dataset_qa(examples, tokenizer, max_seq_length=None):
    ...
    # augment data to reduce dependence on finding similar words / structure to question (try swapping words with synonyms)
    # transformation = WordSwapEmbedding()
    # constraints = [RepeatModification(), StopwordModification()]
    # augmenter = Augmenter(transformation=transformation, constraints=constraints, pct_words_to_swap=0.5)

    # try swapping word order around
    transformation = WordInnerSwapRandom()
    augmenter = Augmenter(transformation=transformation, pct_words_to_swap=0.7)

    # use the uncommented data augmenter to augment the data
    context = [augmenter.augment(c) for c in examples['context']]
